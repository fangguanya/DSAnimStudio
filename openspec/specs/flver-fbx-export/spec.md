...
### 需求:正式骨骼交付物
模型导出必须生成可被 UE5 正式导入为 SkeletalMesh 的最终交付物，而不是仅供调试的中间文件。

#### 场景:导出结果必须保留骨骼蒙皮能力
- **当** 一个角色模型被标记为导出完成
- **那么** 最终交付物必须保留骨骼层级、骨骼权重、材质槽和网格绑定关系，能够在 UE5 中创建 `USkeletalMesh`、`USkeleton` 和 `UPhysicsAsset`

#### 场景:禁止将Collada静态结果视为完成
- **当** 导出链路回退到 Collada/DAE 或其他不保留正式骨骼蒙皮导入能力的格式
- **那么** 该结果只能视为调试输出，不得计入角色的正式导出完成状态

#### 场景:最终模型交付物唯一
- **当** 未启用显式调试开关
- **那么** 每个角色的 Model 目录必须只保留一套最终交付模型文件及其必要伴随文件；多套中间格式不得默认一并保留并计入正式交付体积
