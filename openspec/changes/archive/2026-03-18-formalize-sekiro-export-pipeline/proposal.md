...
当前 Sekiro 正式导出链路虽然已经能产出可用于验证的模型、动画、纹理和技能配置，但仍然依赖多格式降级导出、兼容式 HKX 读取、导出后修补、纹理解码回退、可选模板和 hack 参数解析等非正式实现。更关键的是，现有规范还没有正确表达 DSAnimStudio 的真实运行语义：编辑器里选中的 TAE 动画条目并不直接等于最终播放的 HKX，主角画面也不是单一 c0000 模型，而是主模型骨架驱动下的规范化多部件装配结果。继续在这条链路上叠加功能，只会把“能跑”和“正式正确”混在一起，导致交付格式、失败语义和 UE 导入契约长期不稳定。
...
- 将“TAE 选择条目 → 引用链求解 → 实际 HKX 动画 → 规范化交付动画文件”的解析关系纳入正式合同，并明确区分动画源 ANIBND 与技能源 ANIBND
- 将主角等多部件角色的正式模型定义为规范化装配结果，而不是 DSAnimStudio 当前 UI 预览状态或单一源 FLVER
...
- `formal-animation-resolution`: 定义 TAE 动画条目、引用链、`SourceAnimFileName` 与最终 HKX/交付动画之间的正式解析关系
...
- `skill-config-export`: 现有技能导出规范需要从“按当前 TAE 条目直接输出”改为“输出可追溯的 TAE 入口、求解后的 HKX 关系和最终交付文件关系”
