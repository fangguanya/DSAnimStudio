...
- [x] 15.1 统一导出器与 UE 导入器的 skill config schema，确定唯一 canonical JSON 结构（`characters[]`、`animations[]`、`frameCount`、`frameRate`、`fileName`、`startFrame`、`endFrame`、`params`）并同步更新 C# 与 UE5 两端
- [x] 15.2 将 `ParamExporter` 正式接入 CLI 全量导出链，确保 `AtkParam`、`BehaviorParam`、`SpEffectParam`、`EquipParamWeapon` 和义手覆盖配置进入最终 skill config 交付物
- [x] 15.3 修复 SkillConfigExporter 的事件时间字段，输出帧域 `startFrame/endFrame`，并补齐 `frameCount/fileName/frameRate`
- [x] 15.4 为全部 185 种 TAE 事件建立正式参数名映射；对模板缺失字段输出数据类型与偏移，不允许静默以占位名掩盖缺口
...
- [x] 15.8 正规化 UE 自定义 glTF 动画导入器，补齐对正式输入格式的解析约束；若支持 `.glb`，必须实现真实的 GLB 容器解析而非按 JSON 读取
- [x] 15.9 收紧 `SekiroValidationCommandlet` 的通过标准：StaticMesh 降级、缺失 Skeleton、缺失 PhysicsAsset、未绑定动画、零轨道动画均应视为错误
- [x] 15.10 扩展 `USekiroSkillDataAsset` / 参数缓存层，使 UE 端保存可复刻的结构化战斗数据，而不是仅保存字符串化参数映射
...
- [x] 15.15 将“禁止猜测/临时/兼容/替代/简化实现进入正式链路”写入 proposal/design/spec，作为持续生效的实现约束
- [x] 15.16 用 Sekiro 正式参数表 + 受规范约束的 canonical export profile 替换 CLI 中 `c0000` 的猜测性默认部件装配，并让 `material_manifest.json` 与最终多部件模型一致
- [x] 15.17 移除动画正式链路中的伪造载体网格、导出后修补和其它临时补丁，改为单文件正式 clip 输出实现
