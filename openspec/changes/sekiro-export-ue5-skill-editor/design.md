...
### 决策 9：正式链路禁止使用猜测/临时/兼容/替代/简化实现

**选择：** 只要某段逻辑属于正式导出/导入/预览链路，就必须建立在游戏原始数据、参数表、规范配置、明确格式契约或已验证的正式转换规则之上；不允许继续依赖角色特判硬编码、伪造载体几何、导出后修补脚本、宽松 fallback、猜测性默认值或其它“先跑通再说”的实现。

**替代方案考虑：**
- **保留临时补丁，后续再清理**：短期能继续推进，但会让“正式链路”长期混入不可验证的行为，验收标准失真
- **对缺失正式来源的数据做猜测性补全**：看起来可用，但会把数据错误伪装成成功结果，后续无法追责到真实输入

**理由：** 本变更现在已经从原型验证阶段转入正式交付阶段。只要继续允许猜测、临时、兼容或替代逻辑混入正式链路，就无法证明输出资产与游戏真实数据、导入契约和预览结果一一对应。
